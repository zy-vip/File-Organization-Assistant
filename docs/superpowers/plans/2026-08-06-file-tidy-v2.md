...
**Interfaces:**
- Consumes: `Rule`（现有）
- Produces: `ProFeature`（枚举：`RegularExpression`、`RenameTemplate`、`DuplicateDetection`）、`RuleAction`（抽象基类，`[JsonDerivedType]` 注册 `move`/`moveRename`）、`MoveAction`、`MoveAndRenameAction`（`Template`）、`Rule.Actions`（`List<RuleAction>`）、`Rule.EffectiveAction`（`Actions.Count>0 ? Actions[0] : new MoveAction()`；多余 Action（仅手工改配置可能出现）忽略——Core 无日志设施，设计要求的"记日志"以忽略行为近似，不加额外实现）、`FileCondition.RequiredFeature`（虚属性 `ProFeature?`，默认 null）
...
- Produces: `PreviewStatus` 新增 `TemplateError`、`NeedsPro`；`PreviewEntry.BlockedFeature`（string?，NeedsPro 时记录所需 Pro 功能中文名，`/` 分隔）；`PreviewService.Build(IReadOnlyList<Rule>, IEnumerable<FileEntry>, DateTime, Func<ProFeature, bool>? isAllowed = null)`（isAllowed 缺省 null=全部放行，保持既有调用兼容）
...
预期：`tools/FileTidy.LicenseTool/private_key.pem` 生成（PKCS#8 PEM），控制台输出公钥 PEM。

先验证「为空」分支（此时公钥尚未填入）：

Run: `dotnet run --project tools/FileTidy.LicenseTool -- validate FTID-AAAA-BBBB`
预期：错误提示「LicenseKeys.AppPublicKeyPem 为空，请先执行 keygen 并填入公钥」。

然后用编辑器把公钥 PEM（含 `-----BEGIN PUBLIC KEY-----` 与 `-----END PUBLIC KEY-----` 首发首尾行）填入 `src/FileTidy.Core/LicenseKeys.cs` 的 `AppPublicKeyPem` 常量字符串中。

填入后 Run: `dotnet run --project tools/FileTidy.LicenseTool -- validate FTID-AAAA-BBBB`
预期：输出「无效」（假码被正式公钥拒绝，返回码 1）。
...
- Produces: `Organizer.Execute` 对 `NeedsPro` 条目记 Skipped（原因「需要 Pro 解锁（{BlockedFeature}）」）、对 `TemplateError` 记 Failed（原因「模板错误」）；`MainViewModel` 构造新增可选参数 `LicenseService? license`（缺省 null 时内置正式密钥 + AppPaths 路径构造，保证测试与生产行为一致但可注入临时）
...
- Produces: `RuleEditorViewModel` 新增属性：`RegexPattern`、`RegexCaseSensitive`（bool，默认 false，勾选=区分大小写）、`ActionType`（string，`move`/`moveRename`）、`RenameTemplate`；校验新增：正则非法、模板结构非法、动作与模板联动（选中 moveRename 时模板必须通过 Validate）；`ApplyToModel` 写 RegexCondition / Actions；`MainViewModel.FromRule` 反向填充
...
LicenseTool 单独发布（开发者本机使用，不随商品发布）：

```powershell
dotnet publish tools/FileTidy.LicenseTool -c Release -o tools/FileTidy.LicenseTool/dist
```

Expected: 两者各自产出 exe 且可运行；`dist/FileTidy.exe` 可启动、LicenseTool exe 可 `generate` 出激活码（工作目录含 `private_key.pem`）。

- [ ] **Step 5: 手动验收（对照设计文档第 7 章 10 项清单）**
